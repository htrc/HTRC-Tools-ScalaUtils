...
object CollectionsImplicits {
  ...
  implicit class IteratorWithGroupConsecutiveWhen[A: ClassTag](s: Iterator[A]) {
    ...
    def groupConsecutiveWhen[C[X]](p: (A, A) => Boolean)
                               (implicit cbf: CanBuildFrom[_, A, C[A]]): Iterator[C[A]] = new AbstractIterator[C[A]] {
  ...
  implicit class StringSeqWithDehyphenate[C[_] <: SeqLike[String, C[String]]](s: C[String]) {
    /**
      * Dehyphenates a set of lines of text (i.e. joins end-of-line hyphenated words)
      * Loosely follows the rules specified at http://englishplus.com/grammar/00000129.htm
      *
    ...
    def dehyphenate(allowedHyphenChars: String = "-‐‑‒–―−")
                   (implicit cbf: CanBuildFrom[Iterator[String], String, C[String]]): C[String] = {
      new EndOfLineDehyphenator(s.iterator, allowedHyphenChars).to[C]
    }
  }
  ...
  implicit class SeqWithIsMonotonic[A](seq: Seq[A]) {
    /**
  ...
  implicit class TraversableOnceWithMinMaxByOpt[A](t: TraversableOnce[A]) {
    /**
