package org.hathitrust.htrc.tools.java.hashing;
 ...
 *
 * <p>Let S be a string of bytes and <i>g(S)</i> the string obtained by taking the
 ...
 * long word.
 *
 * <p>Fingerprints can be used as a probably unique id for the input string. More
 ...
 *
 * <p>The routines named <code>extend[8]</code> and <code>fp[8]</code> return
 ...
 *
 * <p>Source: http://www.crawler.archive.org/apidocs/st/ata/util/FPGenerator.html
 */
...
public final class FPGenerator {
  ...
  private FPGenerator(long polynomial, int degree) {
    ...
    long xToTheI = one;
    long xToTheDegreeMinusOne = (one >>> (degree - 1));
    for (int i = 0; i < 128; i++) {
      // Invariants:
      // xToTheI = mod(x^i, polynomial)
      // forall 0 <= j < i, powerTable[i] = mod(x^i, polynomial)
      powerTable[i] = xToTheI;
      boolean overflow = ((xToTheI & xToTheDegreeMinusOne) != 0);
      xToTheI >>>= 1;
      if (overflow) {
        xToTheI ^= polynomial;
      }
