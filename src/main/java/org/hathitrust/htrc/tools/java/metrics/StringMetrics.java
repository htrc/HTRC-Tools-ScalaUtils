package org.hathitrust.htrc.tools.java.metrics;

/**
 * Provides methods for computing string metrics.
 */
public final class StringMetrics {
   ...
   * word into the other.
   *
   * <p>This is a fast iterative algorithm that's also memory efficient.
   * O(len1 * len2) time, O(len1) space Source: https://en.wikipedia.org/wiki/Levenshtein_distance
   *
  ...
  public static int levenshteinDistance(final String s0, final String s1) {
    ...
    for (int j = 0; j < len1; j++) {
      ...
      for (int i = 0; i < len0; i++) {
        ...
        int costReplace = cost[i] + match;
        int costInsert = cost[i + 1] + 1;
        int costDelete = newcost[i] + 1;
